 ...
 */
const ArticlesNext: React.FC<ArticlesNextProps> = ({
  articles,
  pathPrefix,
}) => {
  ...
  return (
    <Grid numberOfArticles={numberOfArticles}>
      <GridItem article={articles[0]} pathPrefix={pathPrefix} />
      <GridItem article={articles[1]} narrow pathPrefix={pathPrefix} />
    </Grid>
  );
};
...
const GridItem: React.FC<GridItemProps> = ({ article, narrow, pathPrefix }) => {
  if (!article) return null;

  const hasOverflow = narrow && article.title.length > 35;
  const imageSource = narrow ? article.poster.narrow : article.poster.regular;

  return (
    <ArticleLink
      to={`${pathPrefix}${article.slug}`}
      data-a11y="false"
    ...
    >
      <Item>
        ...
        <MetaData>
          ...
          {article.body.childMdx.timeToRead} min read
        </MetaData>{" "}
      </Item>
    </ArticleLink>
  );
